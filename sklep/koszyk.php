    ...
    try {
        ...
        if ($goforit == true) {
            ...
            foreach($_SESSION['produkty'] as $key => $val){
                $stmt = $pdo->exec('INSERT INTO szczegoly_transakcji (`id_transakcji`, `id_produktu`, `ilosc`, `cena`) VALUES ( ' . $last_id . ',' . $val . ',' . $_SESSION['ilosci'][$key] . ',' . $_SESSION['ilosci'][$key]*$_SESSION['ceny'][$key]*$promocode . ')');
            }
            unset($_SESSION['produkty']);
            unset($_SESSION['ilosci']);
            unset($_SESSION['ceny']);
